...
char *print_currently_serving(Ta *ta_list) {
    char *stat = (char *) malloc(sizeof(char) * OUT_BUF_SIZE);
    stat[OUT_BUF_SIZE - 1] = '\0';
    if (ta_list == NULL) {
        strcpy(stat, "No TAs are currently working.\n");
        return stat;
    }
    while (ta_list != NULL) {
       if (ta_list->current_student != NULL) {
           strncat(stat, "TA: ", OUT_BUF_SIZE - strlen(stat) - 1);
           strncat(stat, ta_list->name, OUT_BUF_SIZE - strlen(stat) - 1);
           strncat(stat, " is serving ", OUT_BUF_SIZE - strlen(stat) - 1);
           strncat(stat, ta_list->current_student->name, OUT_BUF_SIZE - strlen(stat) - 1);
           strncat(stat, ".\n", OUT_BUF_SIZE - strlen(stat) - 1);
       } else {
           strncat(stat, "TA: ", OUT_BUF_SIZE - strlen(stat) - 1);
           strncat(stat, ta_list->name, OUT_BUF_SIZE - strlen(stat) - 1);
           strncat(stat, ", has no student\r\n", OUT_BUF_SIZE - strlen(stat) - 1);
       }
...
char *print_full_queue(Student *stu_list) {
    char *stat = (char *) malloc(sizeof(char) * OUT_BUF_SIZE);
    stat[OUT_BUF_SIZE - 1] = '\0';
    strcpy(stat, "Full Queue\n");
    while (stu_list != NULL) {
        strncat(stat, "Student ", OUT_BUF_SIZE - strlen(stat) - 1);
        strncat(stat, stu_list->name, OUT_BUF_SIZE - strlen(stat) - 1);
        strncat(stat, ":", OUT_BUF_SIZE - strlen(stat) - 1);
        strncat(stat, stu_list->course->code, OUT_BUF_SIZE - strlen(stat) - 1);
        strncat(stat, "\n", OUT_BUF_SIZE - strlen(stat) - 1);
        stu_list = stu_list->next_overall;
    }
    return stat;
